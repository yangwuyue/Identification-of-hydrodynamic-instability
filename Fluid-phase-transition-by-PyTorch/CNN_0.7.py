...
TRAIN_ROOT = "D:/peng/tcflow0716"
TEST_ROOT = "D:/peng/tcflow0716"

TRAIN_PARAMETER = {1: (create_directories(TRAIN_ROOT, 37.275, 68.25), range(1800, 2010, 20)),
                   0: (create_directories(TRAIN_ROOT, 80.85, 106.575), range(1800, 2010, 20))}

TEST_PARAMETER = {1: (create_directories(TEST_ROOT, 68.25, 79.38), [2000]),
                  0: (create_directories(TEST_ROOT, 79.59, 106.575), [2000])}
...
def train():
    ...
    torch.save(net, '0716_model.pkl')

    output = open("0716_loss_history.txt", "w")
    for index in range(len(history["loss"])):
        output.write(str(history["loss"][index]) + " ")
    output.close()

    output = open("0716_acc_history.txt", "w")
...
if __name__ == "__main__":
    ...
    # train()
    le_net = load_model("0716_model.pkl")
