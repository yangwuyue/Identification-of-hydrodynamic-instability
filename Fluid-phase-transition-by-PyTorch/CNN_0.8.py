...
TRAIN_ROOT = "/Users/app/Desktop/scientificresearch/tcflow019"
TEST_ROOT = "/Users/app/Desktop/scientificresearch/tcflow019"

TRAIN_PARAMETER = {1: (create_directories(TRAIN_ROOT, 22.4, 75.2), range(1800, 2010, 10)),
                   0: (create_directories(TRAIN_ROOT, 121.6, 148.8), range(1800, 2010, 10))}

# print("Train data size")
TEST_PARAMETER = {1: (create_directories(TEST_ROOT, 75.2, 96.0), [2000]),
                  0: (create_directories(TEST_ROOT, 97.6, 156.8), [2000])}
...
def train():
    ...
    torch.save(net, '091_model.pkl')

    output = open("091_loss_history.txt", "w")
    for index in range(len(history["loss"])):
        output.write(str(history["loss"][index])+" ")
    output.close()

    output = open("0.8_acc_history.txt", "w")
...
def show(truth_value, predict_value):
    ...
    '''
    for index in range(len(predict_value)):
        if predict_value[index] != predict_value[index + 1]:
            plt.plot([index + 1, index + 1], [0, 1], 'r')
            break
    '''

    # print("predict turn point: ", dir_name_list[index])
    ...
    '''
    for index in range(len(truth_value)):
        if truth_value[index] != truth_value[index+1]:
            plt.plot([index+1, index+1], [0, 1], 'g')
            break
    '''
...
def show(path):
    ...
    value = []
    for item in content[0].strip().split():
...
if __name__ == "__main__":
    ...
    #train()
    #le_net = load_model("091_model.pkl")
    #t, p = validate(model=le_net, epoch=EPOCH, result=True)
    #show(t, p)
    #show_history()
    show('/Users/app/Desktop/Fluid_phase_transition/Fluid_phase_transition_by_PyTorch/091_acc_history.txt')
