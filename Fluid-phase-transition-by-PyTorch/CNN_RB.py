...
def default_loader(image_path):
    ...
    t_path, u_path = image_path
    t_file = open(t_path, "r").readlines()
    for t_index in range(len(t_file)):
        if t_file[t_index][0] == "(":
            t_start = t_index + 1
            break
    t_data = []
    for index in range(WIDTH*HEIGHT):
        t_data.append(float(t_file[t_start + index].strip()))
    ...
    for index in range(WIDTH*HEIGHT):
        ...
        data.append([t_data[index], pow(pow(float(values[0][1:]), 2)+pow(float(values[1]), 2), 0.5)])
        ...
        for label in [1, 0]:
            for dirs in parameter[label][0]:
                for file_name in parameter[label][1]:
                    t_path = os.path.join(root, dirs + "/T" + str(file_name) + ".dat")
                    u_path = os.path.join(root, dirs + "/U" + str(file_name) + ".dat")
                    if os.path.isfile(t_path) and os.path.isfile(u_path):
                        self.__images.append(((t_path, u_path), label))
...
TRAIN_ROOT = "D:/peng/1010"
TEST_ROOT = "D:/peng/1010"

TRAIN_PARAMETER = {1: (create_directories(TRAIN_ROOT, 1500, 1650), range(3900, 4010, 10)),
                   0: (create_directories(TRAIN_ROOT, 1750, 1899), range(3900, 4010, 10))}

TEST_PARAMETER = {1: (create_directories(TEST_ROOT, 1691, 1708), [4000]),
                  0: (create_directories(TEST_ROOT, 1709, 1790), [4000])}
...
def train():
    ...
    torch.save(net, '1010_model.pkl')

    output = open("1010_loss_history.txt", "w")
    for index in range(len(history["loss"])):
        output.write(str(history["loss"][index]) + " ")
    output.close()

    output = open("1010_acc_history.txt", "w")
...
if __name__ == "__main__":
    ...
    # train()
    le_net = load_model("1010_model.pkl")
